...
/********************************************************************************************************************/
  ...
  {               
    ...
    {   
      Startzeit_Buzzer_Sensor_1_IO_Tester = millis();   //Startzeit Buzzer speichern
      Start_Buzzer_Sensor_1_IO_Tester = true;           //Start Buzzer zurücksetzen
    } 
  }  
...
/********************************************************************************************************************/
  ...
  {               
    ...
    /* Buzzer einschalten */
    if ((Buzzer_Ein_IO_Tester == true) && (Start_Buzzer_Sensor_2_IO_Tester == false))
    {   
      Startzeit_Buzzer_Sensor_2_IO_Tester = millis();   //Startzeit Buzzer speichern
      Start_Buzzer_Sensor_2_IO_Tester = true;           //Start Buzzer zurücksetzen
    }  
  }
...
/********************************************************************************************************************/
  ...
  { 
    /* Buzzer setzen */
    if (((millis()) < (Startzeit_Buzzer_Sensor_1_IO_Tester + 5000)) || ((millis()) < (Startzeit_Buzzer_Sensor_2_IO_Tester + 5000)))
    {
      analogWrite(DA_Buzzer, Buzzer_Lautstaerke);     //Buzzer setzen (0=Aus, 5-255=Ein) 
    }   

    /* Ton ändern wenn beide Sensoren eingeschalten sind */
  ...
  /* Buzzer ausschalten */
  if ((Buzzer_Ein_IO_Tester == false) || ((digitalRead(DE_Sensor_1) == LOW) && (digitalRead(DE_Sensor_2) == LOW)))
  {    
    digitalWrite(DA_Buzzer, LOW);                     //Buzzer zurücksetzen
    Start_Buzzer_Sensor_1_IO_Tester = false;          //Start Buzzer zurücksetzen
    Start_Buzzer_Sensor_2_IO_Tester = false;          //Start Buzzer zurücksetzen 
